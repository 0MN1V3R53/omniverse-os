<?php
/**
 * calculate_quote.php — Sky Auto Services Master Pricing Engine v3 (Security Hardened)
 * Omniverse Security Hardened Suite | CISO Michael Chang & Priya Patel
 */
...
if (!$body || !is_array($body)) { http_response_code(400); echo json_encode(['success'=>false,'error'=>'Invalid JSON payload']); exit; }
...
$orig=strip_tags(trim($body['origin']??''));
$dest=strip_tags(trim($body['destination']??''));
$oA=gAbbr($orig,$SA,$SP);
$dA=gAbbr($dest,$SA,$SP);
...
// Validate Coordinates Strictly (Prevent SSRF)
$raw_oLat = $body['originGeo']['lat'] ?? null;
$raw_oLon = $body['originGeo']['lon'] ?? null;
$raw_dLat = $body['destGeo']['lat'] ?? null;
$raw_dLon = $body['destGeo']['lon'] ?? null;

$oLat = is_numeric($raw_oLat) && $raw_oLat >= -90 && $raw_oLat <= 90 ? floatval($raw_oLat) : null;
$oLon = is_numeric($raw_oLon) && $raw_oLon >= -180 && $raw_oLon <= 180 ? floatval($raw_oLon) : null;
$dLat = is_numeric($raw_dLat) && $raw_dLat >= -90 && $raw_dLat <= 90 ? floatval($raw_dLat) : null;
$dLon = is_numeric($raw_dLon) && $raw_dLon >= -180 && $raw_dLon <= 180 ? floatval($raw_dLon) : null;
    ...
    if (!empty($oM[1]) && !empty($dM[1])) {
        ...
        if (file_exists($zipDataFile)) {
            ...
            if (isset($zipData[$oM[1]]) && isset($zipData[$dM[1]])) {
                $oLat = floatval($zipData[$oM[1]]['lat']);
                $oLon = floatval($zipData[$oM[1]]['lon']);
                $dLat = floatval($zipData[$dM[1]]['lat']);
                $dLon = floatval($zipData[$dM[1]]['lon']);
            }
        }
    }
}
...
$bc*=$sm;
$vt=isset($VF[$body['vehicleType']??'']) ? $body['vehicleType'] : 'sedan';
$vs=$VF[$vt]??0; $bc+=$vs;
$is=0; if(($body['vehicleCondition']??'')==='inoperable'){$is=150;$bc+=150;}
$tt=isset($TM[$body['transportType']??'']) ? $body['transportType'] : 'open_standard';
$tmult=$TM[$tt]??1.00; $tmin=$TMN[$tt]??399;
